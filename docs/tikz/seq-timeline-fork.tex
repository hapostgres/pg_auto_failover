...
\node[note, fill=async, draw=gray!50, text width=15cm, font=\small]
  at (9.5, 19.6)
  {reported state never actually changed (``secondary'' throughout), so no
   FSM transition fires -- steady-state streaming never re-checks ancestry.
   The fork is durably published to
   \texttt{pgautofailover.node\_timeline\_history} but otherwise goes
   unnoticed};

%% ── forced through a real transition ─────────────────────────────────────
\node[seclbl] at (-3.4,21.4) {forced through a real transition};

\draw[msg] (\mx,22.3) -- (\sx,22.3);
\node[lbl, above] at (\mx-3.4,22.3) {goal=maintenance};
\draw[msg] (\mx,23.5) -- (\sx,23.5);
\node[lbl, above] at (\mx-3.2,23.5) {goal=catchingup};

\node[note, fill=gray!12, draw=gray!55, text width=13cm, font=\small]
  at (9.5, 24.7)
  {any ordinary ``briefly unreachable, please resync'' health-check cycle
   drives the same transition -- an operator maintenance cycle is used
   here only to trigger it deterministically};
