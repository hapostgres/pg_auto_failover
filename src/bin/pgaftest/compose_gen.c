...
#define SSL_COPY_CERTS_CMD \
	...
	" && cp " SSL_DIR_IN_CONTAINER "/server/server.key /var/lib/postgres/server.key" \
	" && chmod 0600 /var/lib/postgres/server.key" \
	" && mkdir -p /var/lib/postgres/.postgresql" \
	" && cp " SSL_DIR_IN_CONTAINER "/client/postgresql.crt /var/lib/postgres/.postgresql/postgresql.crt" \
	" && cp " SSL_DIR_IN_CONTAINER "/client/postgresql.key /var/lib/postgres/.postgresql/postgresql.key" \
	" && chmod 0600 /var/lib/postgres/.postgresql/postgresql.key" \
	" && cp " SSL_DIR_IN_CONTAINER "/ca.crt /var/lib/postgres/.postgresql/root.crt" \
	" &&"
                  ...
                  const char *specFile)
{
	...
	if (cluster->withMonitor)
	{
		...
		if (cluster->monitorDebianCluster[0])
			snprintf(monitor_pgdata, sizeof(monitor_pgdata),
	...
	for (int fi = 0; fi < cluster->formationCount; fi++)
	{
		...
		for (int ni = 0; ni < form->nodeCount; ni++)
		{
			...
			if (n->debianCluster[0])
				snprintf(node_pgdata, sizeof(node_pgdata),
				         DEBIAN_PGDATA_PREFIX "/%s/%s",
	...
	if (specFile)
	{
		...
		if (slash) *slash = '\0'; else strlcpy(workDir, ".", sizeof(workDir));
		...
		fprintf(f,
			...
			workDir, projectName);
		if (cluster->withMonitor)
			fprintf(f, "      PG_AUTOCTL_MONITOR: \"%s\"\n", monitorPguri);
		fprintf(f, "    command: [\"pgaftest\", \"run\", \"/spec.pgaf\"]\n\n");
	}
...
compose_gen_write_monitor_ini(const TestCluster *cluster, const char *dir)
{
	...
	if (cluster->monitorDebianCluster[0])
		snprintf(mon_pgdata, sizeof(mon_pgdata),
		         DEBIAN_PGDATA_PREFIX "/%s/%s",
		         debian_pg_version(), cluster->monitorDebianCluster);
	else
