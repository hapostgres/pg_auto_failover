 ...
 */
typedef struct { const char *kw; char val[128]; } NodeProp;
...
#define ADD(k, v)  do { props[pc].kw = (k); strlcpy(props[pc].val, (v), sizeof(props[0].val)); pc++; } while (0)
#define ADDF(k)    do { props[pc].kw = (k); props[pc].val[0] = '\0'; pc++; } while (0)
	if (n->launchDeferred) ADDF("launch deferred");
	if (n->async)          ADDF("async");
	if (n->noMonitor)      ADDF("no-monitor");
	if (n->listen)         ADDF("listen");
	if (n->candidatePriority != 50)
	{ snprintf(numbuf, sizeof(numbuf), "%d", n->candidatePriority); ADD("candidate-priority", numbuf); }
	if (!n->replicationQuorum) ADD("replication-quorum", "false");
	if (n->citusSecondary)     ADDF("citus-secondary");
	if (n->citusClusterName[0]) ADD("citus-cluster-name", n->citusClusterName);
	if (n->pgPort != 0)
	{ snprintf(numbuf, sizeof(numbuf), "%d", n->pgPort); ADD("port", numbuf); }
	if (n->ssl[0])  ADD("ssl",           n->ssl);
	if (n->auth[0]) ADD("auth",          n->auth);
	if (n->debianCluster[0]) ADD("debian-cluster", n->debianCluster);
...
#undef ADD
	...
	for (int i = 0; i < pc; i++)
	{
		...
		strncat(inline_props, props[i].kw, sizeof(inline_props)-strlen(inline_props)-1);
		if (props[i].val[0])
		{
			strncat(inline_props, " ", sizeof(inline_props)-strlen(inline_props)-1);
			strncat(inline_props, props[i].val, sizeof(inline_props)-strlen(inline_props)-1);
		}
	}
