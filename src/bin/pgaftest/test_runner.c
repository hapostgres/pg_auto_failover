		...
		for (int ni = 0; ni < form->nodeCount; ni++)
		{
			...
			if (groupCount > 0)
			{
				...
				for (int gi = 0; gi < groupCount; gi++)
					if (node->group == groupIds[gi]) { inGroup = true; break; }
				if (!inGroup)
					continue;
			}
                              ...
                              int timeoutSecs)
{
	...
	for (int i = 0; i < stateCount; i++)
		if (!satisfied[i]) { allSatisfiedEarly = false; break; }
...
wait_for_states(TestRunner *r, TestCmd *cmd)
{
	...
	{
		if (i > 0) strncat(label, ", ", sizeof(label) - strlen(label) - 1);
		...
		case CMD_WAIT_MULTI:
		{
			...
			while (time(NULL) < deadline)
			{
				...
				if (subproc_ok == 0)
				{
					...
					for (int i = 0; i < cmd->waitStateCount; i++)
						if (!listenSatisfied[i]) { allListenMet = false; break; }
					if (allListenMet)
						return true;
				}
				else if (allMet)
...
cmd_label(const TestCmd *cmd, char *buf, int len)
{
	...
	switch (cmd->kind)
	{
		...
		case CMD_WAIT_STATES:
		{
			...
			for (int i = 0; i < cmd->waitStateCount; i++)
			{
				if (i > 0) strncat(states, ", ", sizeof(states)-strlen(states)-1);
		...
		case CMD_PROMOTE:
		{
			...
			for (int i = 0; i < cmd->promoteCount; i++)
			{
				if (i > 0) strncat(nodes, ", ", sizeof(nodes)-strlen(nodes)-1);
		...
		case CMD_WAIT_MULTI:
		{
			...
			for (int i = 0; i < cmd->waitStateCount; i++)
			{
				if (i > 0) strncat(parts, " and ", sizeof(parts)-strlen(parts)-1);
				char piece[128];
...
runner_prepare(TestSpec *spec, const char *outDir)
{
	...
	if (!outDir || outDir[0] == '\0')
	{
		...
		if (dot && strcmp(dot, ".pgaf") == 0) *dot = '\0';
