 ...
 *   pg_basebackup's own client-side tar handling and the backend's
 *   basebackup.c both build on. pg_walsender's tar_stream.c uses
 *   tarCreateHeader()/tarPaddingBytesRequired() directly rather than
 *   re-deriving the ustar byte layout by hand.
 *
