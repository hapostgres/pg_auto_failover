 ...
 *
 * -------------------------------------------------------------------------
 * Vendored from PostgreSQL's src/bin/pg_basebackup/pg_receivewal.c
 * (REL_14_STABLE, not a later branch: REL_17_STABLE's own walmethods.h
 * pulls in common/compression.h, which doesn't exist as of PG14's own
 * header tree -- this project rebuilds pg_autoctl fresh against each of
 * PG14 through PG19's own headers, so the vendored copy has to compile
 * against all of them; PG14's simpler, pre-compression-refactor source
 * does, and the replication protocol itself is unaffected by that
 * refactor, so this still talks to a PG17/18/19 primary exactly the same
 * way a real, unmodified pg_receivewal built from any of those versions
 * would) for src/bin/pg_autoctl's own archiver -- see this project's own
 * service_archiver_pgreceivewal_ctl.c for why: an in-process copy is
 * what makes it possible to hook WAL-segment completion (below) without
 * a second wire protocol or shelling out to the external binary. Two
 * changes from upstream, both marked "PGAF:" at their exact location,
 * everything else byte-for-byte unchanged:
 *   1. main() renamed to pg_receivewal_main() and no longer static/the
 *      process entry point -- called directly, once per (re)connection
 *      cycle, from service_archiver_pgreceivewal_ctl.c's own forked child
...
/*
 * PGAF: the hook itself -- a plain function pointer, NULL by default (a
 * pg_receivewal_main() caller that never sets it gets stock upstream
 * behavior, byte for byte). Set once, right after fork() and before
 * calling pg_receivewal_main(), by service_archiver_pgreceivewal_ctl.c's
 * own child -- safe as a plain global precisely because that's a genuine
 * fork()ed child process, not a thread sharing state with anything else.
 */
...
stop_streaming(XLogRecPtr xlogpos, uint32 timeline, bool segment_finished)
{
	...
	/*
	 * PGAF: fire the hook before any of this function's own pre-existing
	 * logic below -- segment_finished is already exactly the "just closed"
	 * signal (see stream_stop_callback's own comment, receivelog.h); xlogpos
	 * here is the position stop_streaming() was called *at* i.e. the just-
	 * completed segment's own end boundary.
	 */
...
/*
 * PGAF: PG19 changed pqsignal()'s own handler signature to take a second
 * "const pg_signal_info *" parameter (this vendored copy predates that,
 * see this file's own header comment) -- this thin adapter is the
 * version-gated bridge, compiled in only against PG19+ headers, that lets
 * sigint_handler() above keep its simple pre-19 signature unchanged.
 */
static void
sigint_handler_pg19(int signum, const pg_signal_info *si)
{
