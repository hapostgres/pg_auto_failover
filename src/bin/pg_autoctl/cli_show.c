...
static int cli_show_timeline_getopts(int argc, char **argv);
static void cli_show_timeline(int argc, char **argv);
...
CommandLine show_timeline_command =
	make_command("timeline",
				 "Show the timeline history known to the monitor for a group, "
				 "and each node's position against it",
				 " [ --pgdata ] --formation --group",
				 "  --pgdata      path to data directory	 \n"
				 "  --monitor     pg_auto_failover Monitor Postgres URL\n"
				 "  --formation   formation to query, defaults to 'default'\n"
				 "  --group       group to query formation, defaults to 0\n",
				 cli_show_timeline_getopts,
				 cli_show_timeline);
...
/*
 * cli_show_timeline_getopts parses the command line options for the
 * command `pg_autoctl show timeline`.
 */
static int
cli_show_timeline_getopts(int argc, char **argv)
{
	KeeperConfig options = { 0 };
	int c, option_index = 0, errors = 0;
	int verboseCount = 0;
...
/*
 * cli_show_timeline prints the group's known timeline history and each
 * node's current position against it.
 */
static void
cli_show_timeline(int argc, char **argv)
{
	KeeperConfig config = keeperOptions;
	Monitor monitor = { 0 };
	...
	if (!monitor_print_timeline(&monitor, config.formation, config.groupId))
	{
		log_fatal("Failed to get the group's timeline history "
				  "from the monitor, see above for details");
		exit(EXIT_CODE_MONITOR);
	}
}
