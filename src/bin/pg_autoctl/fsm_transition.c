...
fsm_init_standby(Keeper *keeper)
{
	...
	if (useArchiver)
	{
		...
		/*
		 * --wal-method=none, deliberately: the backup this fetches was
		 * itself generated by run_pg_basebackup() (service_archiver_
		 * basebackup.c) with --wal-method=stream against the *real*
		 * primary, which already embedded every byte of WAL from that
		 * backup's own checkpoint through its own end LSN directly inside
		 * the backup's own pg_wal/ directory on disk -- the archiver's
		 * stored backup is already fully self-contained and replayable on
		 * its own, nothing further to fetch for the gap a plain --wal-
		 * method=none backup would otherwise have. A second, *concurrent*
		 * --wal-method=stream connection here would ask pg_walsender's own
		 * START_REPLICATION handler (cmd_start_replication.c) to background-
		 * stream that same range a second time -- but that handler only
		 * ever knows how to serve from the archiver's own WAL *cache*
		 * (pg_receivewal's own captured segments), which has no relationship
		 * to a specific backup's own embedded pg_wal/ range and can easily
		 * not extend as far back, especially for a freshly-started archiver.
		 * Requesting it anyway is what caused a real, consistently
		 * reproducing CI failure ("requested WAL segment predates this
		 * archiver's captured history") -- this node's own catch-up
		 * streaming (below), against the real primary once bootstrapped,
		 * is what covers everything past the backup's own already-embedded
		 * end LSN, the same as for any other standby.
		 */
