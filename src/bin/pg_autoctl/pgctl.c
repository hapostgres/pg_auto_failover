			 ...
			 const char *pgdata, int pgport, char *listen_addresses)
{
	...
	if (program.returnCode != 0)
	{
		...
		else
		{
			...
			if (!pg_log_startup(pgdata, LOG_INFO))
			{
				log_error("Failed to append Postgres logs");
			}
		}
					   ...
					   const char *replicationTargetLSN)
{
	...
	if (!IS_EMPTY_STRING_BUFFER(replicationTargetLSN))
	{
		appendPQExpBuffer(content,
						...
						const char *replicationTargetLSN)
{
	...
	if (!IS_EMPTY_STRING_BUFFER(replicationTargetLSN))
	{
		snprintf(quotedLSN, sizeof(quotedLSN), "'%s'", replicationTargetLSN);
