...
static int escape_recovery_conf_string(char *destination, const char *recoveryConfString);
									  ...
									  GUC *settings)
{
	...
	includeLine = regexp_first_match(currentConfContents,
									 AUTOCTL_CONF_INCLUDE_REGEX);
	if (includeLine != NULL)
	{
		log_debug("%s found in \"%s\"",
				  AUTOCTL_CONF_INCLUDE_LINE, configFilePath);
	...
	log_debug("Adding %s to \"%s\"", AUTOCTL_CONF_INCLUDE_LINE, configFilePath);
	...
	appendPQExpBufferStr(newConfContents, AUTOCTL_CONF_INCLUDE_LINE);
	appendPQExpBufferStr(newConfContents, AUTOCTL_CONF_INCLUDE_COMMENT);
									...
									PostgresSetup *pgSetup)
{
	...
	for (settingIndex = 0; settings[settingIndex].name != NULL; settingIndex++)
	{
		...
		else if (strcmp(setting->name, "port") == 0)
		{
			appendPQExpBuffer(defaultConfContents, "%s = %s\n",
							  setting->name,
							  intToString(pgSetup->pgport).strValue);
		}
		else
		{
			appendPQExpBuffer(defaultConfContents, "%s = %s\n", setting->name,
...
/*
 * pg_write_recovery_conf writes a recovery.conf file to a postgres data directory
 * with the given primary connection info and replication slot name.
 */
bool
pg_write_recovery_conf(const char *pgdata, const char *primaryHost, int primaryPort,
					   const char *replicationUsername, const char *replicationPassword,
					   const char *replicationSlotName)
{
	...
	if (replicationPassword != NULL)
	{
		connInfoEnd += make_conninfo_field_str(connInfoEnd, "password",
											   replicationPassword);
	}
	...
	recoveryConfEnd += escape_recovery_conf_string(recoveryConfEnd, primaryConnInfo);
	recoveryConfEnd += sprintf(recoveryConfEnd, "\nprimary_slot_name = '%s'",
							   replicationSlotName);
